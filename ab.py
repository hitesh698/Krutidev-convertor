...
def update_output(event=None):
    ...
    sections = split_segments(input_box.get("1.0", tk.END))

    for t, txt in sections:
        if t == "hindi":
            output_box.insert(tk.END, txt, "hindi")
        elif t == "eng":
            output_box.insert(tk.END, txt, "english")
        else:
...
def save_docx():
    ...
    doc = Document()
    lines = input_box.get("1.0", tk.END).split("\n")
    ...
    doc.save(file)
    messagebox.showinfo("Saved", "DOCX Saved Successfully!")
...
# ---------------------------------------------------------
root = tk.Tk()
root.title("Unicode → Kruti Dev Converter (FINAL COMPLETE)")
root.geometry("1300x900")

left = tk.Frame(root)
left.pack(side=tk.LEFT, padx=20)

tk.Label(left, text="TYPE HERE:", font=("Arial", 14)).pack()
input_box = scrolledtext.ScrolledText(left, width=60, height=6,
                                      font=("Mangal", 16))
input_box.pack(pady=10)
input_box.bind("<KeyRelease>", update_output)

tk.Label(left, text="Converted Output:", font=("Arial", 14)).pack()
output_box = scrolledtext.ScrolledText(left, width=60, height=10)
output_box.pack(pady=10)

output_box.tag_config("hindi", font=("Kruti Dev 010", 18))
output_box.tag_config("english", font=("Times New Roman", 14))

right = tk.Frame(root)
right.pack(side=tk.RIGHT, padx=40)

tk.Button(right, text="Save DOCX", font=("Arial", 14),
          width=18, height=2, command=save_docx).pack(pady=40)
